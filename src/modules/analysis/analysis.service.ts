...
export class AnalysisService {
  ...
  async analyze(data: AnalyzeRequestDto): Promise<AnalyzeResponseDto> {
    ...
    if (data.budget < snapshot.minBudget) {
      const constraintLatentFactors = { marketAttractiveness: 0, financialFeasibility: 0, riskLevel: 1 };
      ...
      return { score: 0, verdict: 'RED', verdictSource: 'budget_gate', metrics: { demand, marketCapacity, rent, budget, runway, monthlyBurn }, latentFactors: constraintLatentFactors, report: constraintReport };
    }
    ...
    return { score, verdict, verdictSource, metrics: { demand, marketCapacity, rent, budget, runway, monthlyBurn }, latentFactors, report };
  }
}
